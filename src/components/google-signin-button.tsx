'use client'

import { forwardRef, useEffect, useRef, useState } from 'react'
import { motion } from 'framer-motion'
import { Loader2, ArrowRight, Shield } from 'lucide-react'
import { renderGoogleSignInButton } from '@/lib/google-oauth'
import { logger } from '@/lib/logger'

interface GoogleSignInButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  loading?: boolean
  showBenefits?: boolean
  size?: 'sm' | 'md' | 'lg'
  onGoogleCredential?: (idToken: string) => void
}

export const GoogleSignInButton = forwardRef<HTMLButtonElement, GoogleSignInButtonProps>(
  ({ loading = false, showBenefits = true, size = 'lg', className = '', onGoogleCredential, ...props }, ref) => {
    const gisContainerRef = useRef<HTMLDivElement>(null)
    const [gisLoading, setGisLoading] = useState(true)
    const [gisError, setGisError] = useState<string | null>(null)
    const useDirectGoogle = true

    const sizeClasses = {
      sm: 'h-10 text-sm px-4',
      md: 'h-12 text-base px-6',
      lg: 'h-14 text-base px-6'
    }
    
    const iconSize = {
      sm: 'h-4 w-4',
      md: 'h-5 w-5',
      lg: 'h-5 w-5'
    }

    useEffect(() => {
      if (!useDirectGoogle) {
        return
      }

      const handleGsiError = (event: ErrorEvent) => {
        const message = event.message || ''
        if (message.includes('origin is not allowed') || message.includes('GSI_LOGGER')) {
          setGisError(
            'Google sign-in is not configured for this site. Add http://localhost:3000 to Authorized JavaScript origins in Google Cloud Console for your OAuth Web client.'
          )
          setGisLoading(false)
        }
      }

      window.addEventListener('error', handleGsiError)

      if (!gisContainerRef.current || !onGoogleCredential) {
        return () => window.removeEventListener('error', handleGsiError)
      }

      let cancelled = false

      const mountButton = async () => {
        try {
          setGisLoading(true)
          setGisError(null)

          const container = gisContainerRef.current
          if (!container) return

          const width = Math.min(container.offsetWidth || 400, 400)

          await renderGoogleSignInButton(
            container,
            (idToken) => {
              if (!cancelled) {
                onGoogleCredential(idToken)
              }
            },
            { width }
          )
        } catch (error) {
          logger.error('Failed to render Google sign-in button:', error)
          if (!cancelled) {
            const message = error instanceof Error ? error.message : 'Failed to load Google sign-in'
            const isOriginError =
              message.includes('origin is not allowed') ||
              message.includes('origin') ||
              message.includes('403')

            setGisError(
              isOriginError
                ? 'Google sign-in is not configured for this site. Add http://localhost:3000 to Authorized JavaScript origins in Google Cloud Console for your OAuth Web client.'
                : message
            )
          }
        } finally {
          if (!cancelled) {
            setGisLoading(false)
          }
        }
      }

      mountButton()

      return () => {
        cancelled = true
        window.removeEventListener('error', handleGsiError)
      }
    }, [useDirectGoogle, onGoogleCredential])
    
    if (useDirectGoogle) {
      return (
        <div className={showBenefits ? 'space-y-3' : 'space-y-0'}>
          <div className="w-full min-h-[56px] flex items-center justify-center">
            {gisLoading && (
              <Loader2 className="h-6 w-6 animate-spin text-gray-400" />
            )}
            {gisError && (
              <p className="text-sm text-red-500 text-center">{gisError}</p>
            )}
            <div
              ref={gisContainerRef}
              className={`w-full flex justify-center ${gisLoading ? 'hidden' : ''}`}
            />
          </div>

          {showBenefits && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }}
              className="text-center"
            >
              <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center justify-center gap-2">
                <Shield className="h-3 w-3" />
                Secure sign-in with your Google account
              </p>
            </motion.div>
          )}
        </div>
      )
    }

    return (
      <div className={showBenefits ? "space-y-3" : "space-y-0"}>
        <motion.div
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          transition={{ type: 'spring', stiffness: 400, damping: 17 }}
        >
          <button
            ref={ref}
            disabled={loading}
            className={`
              w-full ${sizeClasses[size]} 
              bg-white dark:bg-gray-800 
              border-2 border-gray-200 dark:border-gray-700 
              hover:border-gray-300 dark:hover:border-gray-600 
              hover:shadow-xl dark:hover:shadow-gray-900/30 
              rounded-2xl transition-all duration-300 
              flex items-center justify-center gap-4 
              font-bold text-gray-800 dark:text-gray-200 
              hover:text-gray-900 dark:hover:text-white 
              group disabled:opacity-50 disabled:cursor-not-allowed
              focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500
              active:scale-95 transform
              ${className}
            `}
            {...props}
          >
            {loading ? (
              <Loader2 className={`${iconSize[size]} animate-spin text-gray-400`} />
            ) : (
              <>
                <motion.div
                  className={`${iconSize[size]} flex items-center justify-center`}
                  whileHover={{ rotate: 5 }}
                  transition={{ duration: 0.2 }}
                >
                  <svg
                    className={iconSize[size]}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fill="#4285F4"
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    />
                    <path
                      fill="#34A853"
                      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    />
                    <path
                      fill="#FBBC05"
                      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    />
                    <path
                      fill="#EA4335"
                      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    />
                  </svg>
                </motion.div>
                
                <span className="tracking-wide">Continue with Google</span>
                
                <motion.div
                  className="opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                  initial={{ x: -5 }}
                  animate={{ x: 0 }}
                  transition={{ delay: 0.1 }}
                >
                  <ArrowRight className="h-4 w-4" />
                </motion.div>
              </>
            )}
          </button>
        </motion.div>
        
        {showBenefits && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
            className="text-center"
          >
            <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center justify-center gap-2">
              <Shield className="h-3 w-3" />
              Secure sign-in with your Google account
            </p>
          </motion.div>
        )}
      </div>
    )
  }
)

GoogleSignInButton.displayName = 'GoogleSignInButton'
